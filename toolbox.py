...
def apply_binarization(image: np.ndarray, block_size: int, c_value: int) -> np.ndarray:
    """Aplica binarización adaptativa."""
    if block_size <= 1: 
        block_size = 3
    elif block_size % 2 == 0: 
        block_size += 1
    # Corrección del error 'img is not defined'
    return cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, block_size, c_value)