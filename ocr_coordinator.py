    ...
    def _run_tesseract_task(self, binary_image_for_ocr: np.ndarray, image_file_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.tesseract:
            try:
                ...
                return {
                    "ocr_engine": "tesseract", "processing_time_seconds": 0.0,
                    "recognized_text": {"text_layout": [], "words": []}, "error": str(e)
                }
        return {
            "ocr_engine": "tesseract", "processing_time_seconds": 0.0,
            "recognized_text": {"text_layout": [], "words": []}, "error": "Tesseract not configured"
        }
    ...
    def _run_paddle_task(self, binary_image_for_ocr: np.ndarray, image_file_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.paddle:
            try:
                ...
                return {
                    "ocr_engine": "paddleocr", "processing_time_seconds": 0.0,
                    "recognized_text": {"full_text": "", "lines": []}, "error": str(e)
                }
        return {
            "ocr_engine": "paddleocr", "processing_time_seconds": 0.0,
            "recognized_text": {"full_text": "", "lines": []}, "error": "PaddleOCR not configured"
        }

    def run_ocr_parallel(
        self,
        binary_image_for_ocr: np.ndarray,
        ...
        processing_start_time_coordination = datetime.now() 
        page_dims = {"width": binary_image_for_ocr.shape[1], "height": binary_image_for_ocr.shape[0]} if binary_image_for_ocr is not None else {}
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            future_tesseract = executor.submit(self._run_tesseract_task, binary_image_for_ocr, image_file_name)
            future_paddle = executor.submit(self._run_paddle_task, binary_image_for_ocr, image_file_name)
        ...
        output_data = {
            ...
            "timestamp": processing_start_time_coordination.isoformat(),
        ...
        if not output_data["metadata"]["dimensions"] and binary_image_for_ocr is not None:
            output_data["metadata"]["dimensions"] = {"width": binary_image_for_ocr.shape[1], "height": binary_image_for_ocr.shape[0]}
